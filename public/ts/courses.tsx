...
const CourseButton = ({ quizInfo }: { quizInfo: QuizInfo }) => {
    ...
    return <a href={`/member/quiz/${quizInfo.quiz.id}`} className={`course-item ${completed ? "course-item-completed" : ""}`}>
    ...
    return <>
        ...
        <div id="content">
            <div class="content-centering courses-page">
                <h2>Kurser</h2>
                <p>Här hittar du alla digitala kurser som Stockholm Makerspace har.
                    De flesta kurser sker på plats och annonseras i facebook-gruppen och på slack. Men vissa har en digital variant som du kan ta när du vill.
                </p>
                <div>
                    {courses.map(quizInfo => <CourseButton quizInfo={quizInfo} />)}
                </div>
            </div>
        </div>
    </>
...
common.documentLoaded().then(() => {
    ...
    future1.then((quizzesJson: ServerResponse<QuizInfo[]>) => {
        rootElement.innerHTML = "";
        render(
            <CoursesPage courses={quizzesJson.data} />,
            rootElement
        );
    })
        .catch(json => {

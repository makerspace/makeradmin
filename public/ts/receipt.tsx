...
import { UNAUTHORIZED, ajax, documentLoaded, get_error, show_error } from "./common";
...
const ReceiptWithInfo = ({ transactionId }: { transactionId: number }) => {
    ...
    useEffect(() => {
        ...
        const refresh = async () => {
            try {
                const data = (await ajax("GET", window.apiBasePath + "/webshop/member/current/receipt/" + transactionId, {})).data as ReceiptResponse;
                setReceiptData(data);
            } catch (e) {
                console.error(e);
    ...
    return <>
        ...
        <Receipt transaction={transaction} member={member} detailed={true} />
        {transaction.status === "completed" && <div class="receipt-message">
...
const ReceiptPage = ({ transactionId, productData }: { transactionId: number, productData: ProductData }) => {
...
documentLoaded().then(() => {
    ...
    Promise.all([member, productData]).then(([member, productData]) => {
        if (root != null) {
            root.innerHTML = "";
            render(
                <ReceiptPage transactionId={window.transactionId} productData={productData} />,
                root
            );
        }
    })
        .catch(json => {
            ...
            if (json.status === UNAUTHORIZED) {
                ...
                UIkit.modal.alert("<h2>Misslyckades med att hämta kvittot</h2>" + get_error(json));
            }
        });
});

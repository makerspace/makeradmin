...
common.onGetAndDocumentLoaded(
    "/webshop/product_data/" + window.productId,
    (value: any) => {
        ...
        const productUnit =
            product.smallest_multiple === 1
                ? product.unit
                : `${product.smallest_multiple}${product.unit}`;
        document.getElementById("price")!.innerText =
            `${productPrice} kr/${productUnit}`;

        document
            .getElementById("smallest-multiple")!
            .setAttribute("step", product.smallest_multiple);

        document.getElementById("unit")!.innerText = product.unit;

        document.getElementById("description")!.innerHTML = product.description;

        const productAmount = <HTMLInputElement>(
        ...
        function updateCartSum(cart: Cart) {
            if (cart.items.length > 0) {
                document.querySelector("#cart-sum")!.textContent =
                    " (" + Cart.formatCurrency(cart.sum(id2item)) + ")";
            } else {
                document.querySelector("#cart-sum")!.textContent = "";
            }
        }
        ...
        const image_url = `${apiBasePath}/webshop/image/${
            product.image_id || 0
        }`;

...
const RoundedInputField = ({ value, onChange, min, max, step, placeholder, className }: { value: number, onChange: (value: number) => void, min: number, max: number, step: number, placeholder: string, className?: string }) => {
  ...
  return <input type="text" pattern="[0-9]*" inputMode="numeric" step={step} placeholder={placeholder} value={tempValue} className={className} onClick={e => {
    ...
    <div id={`product-${item.id}`} class="product">
      ...
      <div class="product-input product-line">
        <RoundedInputField
          ...
          onChange={newAmount => {
            cart.setItem(item.id, Cart.adjustItemCount(newAmount, item));
            onChangeCart(cart);
          }}
        />
        ...
        <button type="button" class="number-add uk-button uk-button-small uk-button-primary" data-amount="1"
          onClick={e => {
            cart.setItem(item.id, Cart.adjustItemCount(cart.getItemCount(item.id) + 1, item));
            onChangeCart(cart);
            e.preventDefault();
          }}
        >+</button>

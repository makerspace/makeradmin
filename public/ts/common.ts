...
export function refreshLoggedIn(
    callback: (loggedIn: boolean, permissions: string[] | null) => void,
) {
...
export function onGetAndDocumentLoaded(url: string, callback: any) {
    Promise.all([ajax("GET", window.apiBasePath + url, {}), documentLoaded()])
        .catch((json) => {

...
describe("calculateSpanDiff", () => {
    ...
    expect.extend({
        diffToMatch(
            ...
            const str =
                "diff from spans " +
                JSON.stringify(spanOffsets) +
                " and periods " +
                JSON.stringify(periodOffsets) +
                ", expected";
            ...
            if (!_.isEqual(expectedDeleteIds, actualDeleteIds)) {
                return {
                    message: () =>
                        `${str} deleted ids to be ${JSON.stringify(
                            expectedDeleteIds,
                        )}, was ${JSON.stringify(actualDeleteIds)}`,
                    pass: false,
                };
            }
            const actualAddSpans = addSpans.map((s) => [
                (s.start - today) / DAY_MILLIS,
                (s.end - today) / DAY_MILLIS,
            ]);
            if (!_.isEqual(expectedAddSpans, actualAddSpans)) {
                return {
                    message: () =>
                        `${str} add spans to be ${JSON.stringify(
                            expectedAddSpans,
                        )}, was ${JSON.stringify(actualAddSpans)}`,
                    pass: false,
                };
            }
            ...
            return {
                message: () =>
                    `expected diff to be deletes ${JSON.stringify(
                        expectedDeleteIds,
                    )} and adds ${JSON.stringify(expectedAddSpans)}`,
                pass: true,
            };
        },
    });

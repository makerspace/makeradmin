...
class Test(ApiTest):
    ...
    def test_list_members(self):
        before = self.get("/membership/member?page_size=0").expect(code=200).get('data')
        ...
        after = self.get("/membership/member?page_size=0").get('data')
    ...
    def test_deleted_members_does_not_show_up_in_list(self):
        ...
        after = self.get("/membership/member?page_size=0").expect(code=200).get('data')

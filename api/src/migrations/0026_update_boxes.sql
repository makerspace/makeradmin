...
--- add storage type id to storage items to keep track of what the type the stored item is
ALTER TABLE `storage_items` ADD COLUMN `storage_type_id` int(10) unsigned NOT NULL,;
...
CREATE TABLE `storage_items` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `storage_items_tag_id_index` (`item_label_id`),
...
CREATE TABLE IF NOT EXISTS `storage_messages` (
  ...
  PRIMARY KEY (`id`),
  KEY `item_label_id_key` (`item_label_id`),
  CONSTRAINT `storage_messages_item_label_id_foreign` FOREIGN KEY (`item_label_id`) REFERENCES `storage_items` (`item_label_id`)

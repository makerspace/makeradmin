...
def change_phone_request(member_id: int | None, phone: str) -> int:
    ...
    if member_id is not None:
        ...
        if total_requests > 1024:
            logging.info(f'member id {member_id} change phone number request, too many total requests ({total_requests}) last 30 days')
        ...
        total_requests = db_session.query(PhoneNumberChangeRequest).filter(PhoneNumberChangeRequest.timestamp >= now-timedelta(days=1)).count()

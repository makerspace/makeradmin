...
from box_terminator.models import StorageItem, StorageNag, StorageType, NagType
...
def get_labacess_end_date(item: MemberStorage) -> datetime:
    try:
        return max(
            s.enddate
...
def get_storage_query():
    ...
    query = query.options(
        contains_eager(MemberStorage.member),
        contains_eager(MemberStorage.member).contains_eager(Member.spans),
    )
    return query
...
def get_dates(item: MemberStorage):
    today = date.today()
    ...
    if item.fixed_end_date:
        ...
        to_termination_days = min(
            (terminate_fixed_date - today).days, (terminate_lab_date - today).days
        )
        days_after_expiration = max(
            (today - fixed_end_date).days, (today - expire_lab_date).days
        )
    else:
        fixed_end_date = None
    ...
    pending_labaccess_days = pending_action_value_sum(
        item.member_id, ProductAction.ADD_LABACCESS_DAYS
    )

    return {  # TODO maybe return a class instead?
...
def check_status(dates: Dict[str, datetime]):
    today = date.today()
...
def get_storage_info(item: MemberStorage):
...
def box_terminator_nag(
    member_number=None, item_label_id=None, nag_type=None, description=None
):
    try:
        item = (
            db_session.query(MemberStorage)
            .filter(
                MemberStorage.item_label_id == item_label_id,

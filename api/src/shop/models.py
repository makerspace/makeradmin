...
class Discount(Base):
    ...
    def __repr__(self) -> str:
        return f"Discount(id={self.id}, discount={self.description}, percent_off={self.percent_off}, created_at={self.created_at})"


class DiscountProductMapping(Base):
    __tablename__ = "webshop_discount_product_mapping"

    id = Column(Integer, primary_key=True, nullable=False, autoincrement=True)
    product_id = Column(Integer, ForeignKey("webshop_products.id"), nullable=False)
    discount_id = Column(Integer, ForeignKey("webshop_discounts.id"), nullable=False)

    discount = relationship(Discount, backref="products")
    product = relationship(Product, backref="discounts")

    def __repr__(self) -> str:
        return f"DiscountProductMapping(id={self.id}, product_id={self.product_id}, discount_id={self.discount_id})"

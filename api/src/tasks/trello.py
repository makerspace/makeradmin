...
TEMPLATE_LIST_NAME = "Task Templates"
...
def _fetch_cards_from_trello() -> List[TrelloCard]:
    ...
    params = {"fields": "name,idList,labels,desc", "pluginData": "1", **_auth_params()}
...
def cached_cards(list_name: str) -> List[TrelloCard]:
    ...
    cache_key = f"tasks:cards:{list_name.replace(' ', '_').lower()}"
    ...
    if not list_id:
        raise RuntimeError(f"List '{list_name}' not found on Trello board")
    cards = [c for c in cards if c.idList == list_id]

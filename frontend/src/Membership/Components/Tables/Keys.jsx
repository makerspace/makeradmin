...
module.exports = withRouter(React.createClass({
	...
	renderRow: function(row, i)
	{
		return (
			<tr key={i}>
				<td><Link to={this.state.linkPrefix + "/membership/keys/" + row.key_id}>{row.tagid}</Link></td>
				<td><Link to={"/membership/members/" + row.member_id}>{row.member_number}</Link></td>
				<td><Link to={this.state.linkPrefix + "/membership/keys/" + row.key_id}>{row.description}</Link></td>
				<td>
					<TableDropdownMenu>
						{this.editButton(i)}
						{this.removeButton(i)}
